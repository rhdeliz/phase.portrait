...
#' @param n_bins Integer specifying the number of bins to use for x and y axes. Default is 4.
...
#' @examples
#' plot_n(df, n_bins = 4, input = "standard", min_x = -10, max_x = 10, min_y = -5, max_y = 5, save = TRUE)
#' @export
library(ggplot2)
library(viridis)
library(dplyr)
...
plot_n <- function(df, n_bins = 4, input = "standard", min_x = NULL, max_x = NULL, min_y = NULL, max_y = NULL, save = FALSE) {
  ...
  plot <- df %>%
    ggplot(aes(x, y, color = n, fill = n, label = n)) +
    geom_tile() +                     # Tile fill for each bin based on count `n`
    geom_text(color = "blue") +       # Text overlay to display the count `n`
    ...
    labs(
      ...
      x = df$x_variable[1],                        # x-axis label
      y = df$y_variable[1],                        # y-axis label
  ...
  if (!is.null(min_x) && !is.null(max_x)) {
    plot <- plot + scale_x_continuous(limits = c(min_x - 1/(n_bins), max_x + 1/(n_bins)))
  }

  # Add optional y-axis limits if specified
  if (!is.null(min_y) && !is.null(max_y)) {
    plot <- plot + scale_y_continuous(limits = c(min_y - 1/(n_bins), max_y + 1/(n_bins)))
  }

  # Enforce a fixed aspect ratio for non-fractional plots to maintain shape consistency
  if (input != "fraction") {
  ...
  if (save) {
    ggsave(
      filename = paste0("n_", input, "_", unique(df$x_variable), "_", unique(df$y_variable), ".pdf"),
      plot = plot,
      height = 4, width = 6
    )
  }
