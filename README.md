...
It takes about ten seconds to process and plot a protein pair like the one shown below. Each protein pair plot takes around 200 KB. If running out of memory, set `n_cores = 1`.
...
An example input, as generated by the `lotka_volterra()` function and `lotka_volterra(alpha = 0.75)` for differing conditions, is shown below:
